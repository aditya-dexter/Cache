 ...
 * Removes the element from the cache that is least Frequently used in case cache is full
 * In case two or more element has same (least) frequency, the least recently used element would be removed
 * It internally maintains 2 map, one for key,frequency and another for frequency,elementList(LRU cache)
...
public class CacheLFU<K, V> implements Cache<K, V> {
    ...
    public CacheLFU(Integer capacity){
    ...
    public void put(K key, V value){
        ...
        if(keyFrequencyMap.containsKey(key)){
            ...
            if (thisCount == capacity) {
                //Max capacity reached, free least frequently used node
                freeLeastFreqUsedNode();
            }
    ...
    /**
     * Free least frequently used nodes
     */
    private void freeLeastFreqUsedNode() {
        //get key of last node of the leastFrequency, and then remove from the map and internal cacheLRU
        K key = frequencyNodeListMap.get(leastFrequency).getLastNodeKey();
        removeElement(key);
    ...
    /**
     * Insert new key and value pair, frequency 1
     * @param key
     * @param value
     */
    ...
    private void printCache(){
        System.out.println("CacheMap= " + keyFrequencyMap + ", leastFrequency = " + leastFrequency);
